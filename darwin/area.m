...
- (void)drawRect:(NSRect)r {
  ...
  // no need to save or restore the graphics state to reset transformations;
  // Cocoa creates a brand-new context each time
  (*(a->ah->Draw))(a->ah, a, &dp);
...
- (void)setupNewTrackingArea {
  self->libui_ta = [[NSTrackingArea alloc]
  ...
  if (@available(macOS 10.12, *)) {
    ...
    if (type == NSLeftMouseDown || type == NSRightMouseDown ||
      ...
      me.Count = [e clickCount];
               ...
               type == NSOtherMouseUp) {
      me.Up = buttonNumber;
    } else if (type == NSLeftMouseDragged || type == NSRightMouseDragged ||
...
mouseEvent(mouseMoved) mouseEvent(mouseDragged) mouseEvent(rightMouseDragged)
        mouseEvent(otherMouseDragged) mouseEvent(mouseDown)
            mouseEvent(rightMouseDown) mouseEvent(otherMouseDown)
                mouseEvent(mouseUp) mouseEvent(rightMouseUp)
...
// note: there is no equivalent to WM_CAPTURECHANGED on Mac OS X; there
// literally is no way to break a grab like that even if I invoke the task
// switcher and switch processes, the mouse grab will still be held until I let
// go of all buttons therefore, no DragBroken()
...
- (int)doFlagsChanged:(NSEvent *)e {
  ...
  // Fortunately -[e keyCode] IS valid here, so we can simply map from key code
  // to Modifiers, get the value of [e modifierFlags], and check if the
  // respective bit is set or not — that will give us the up/down state
...
// see
// http://stackoverflow.com/questions/24099063/how-do-i-detect-keyup-in-my-nsview-with-the-command-key-held
...
void uiAreaSetSize(uiArea *a, int width, int height) {
  if (!a->scrolling)
    uiprivUserBug(
        "You cannot call uiAreaSetSize() on a non-scrolling uiArea. (area: %p)",
        a);
  [a->area setScrollingSize:NSMakeSize(width, height)];
}

void uiAreaQueueRedrawAll(uiArea *a) { [a->area setNeedsDisplay:YES]; }

void uiAreaScrollTo(uiArea *a, double x, double y, double width,
                    double height) {
  if (!a->scrolling)
  ...
  // don't worry about the return value; it just says whether scrolling was
  // needed
}
